  ...
  open ? (
    <div style={{ fontSize: 16, color: '#222', lineHeight: 1.8 }}>
      {lang === 'zh'
        ? (
          <>
            <p>Unix 时间戳（Unix timestamp），又称 POSIX 时间，是一种以整数表示时间的方法。它定义为自 1970 年 1 月 1 日 00:00:00 UTC（协调世界时）以来经过的总秒数（不包括闰秒）。<br/>这种表示方式广泛应用于操作系统、数据库、编程语言和互联网服务中，便于时间的存储、比较和计算。</p>
            <div style={{margin:'16px 0 8px 0',fontWeight:500}}>常见问题（FAQ）</div>
            <ol style={{paddingLeft:20}}>
              ...
              <li><b>时间戳会溢出吗？（Year 2038 问题）</b><br/>32 位有符号整数的最大值为 2147483647（对应 2038-01-19 03:14:07 UTC），超过后会溢出，称为“2038 问题”。现代系统多已采用 64 位整数，足以表示数十亿年。</li>
          ...
          <>
            <p>The Unix timestamp (or POSIX time) is an integer representing the number of seconds that have elapsed since January 1, 1970, 00:00:00 UTC (excluding leap seconds).<br/>It is widely used in operating systems, databases, programming languages, and internet services for storing, comparing, and computing time.</p>
            <div style={{margin:'16px 0 8px 0',fontWeight:500}}>FAQ</div>
            <ol style={{paddingLeft:20}}>
              ...
              <li><b>Will the timestamp overflow? (Year 2038 problem)</b><br/>32-bit signed integers overflow at 2147483647 (2038-01-19 03:14:07 UTC). Modern systems use 64-bit integers, which are safe for billions of years.</li>
